...
#define FILE_NAME "test_MN_holding_set_Calibration_DATA.txt"
...
long calibrate(int32_t target_time);
...
void startExperiments(void){
	...
	for(int32_t set_cycle=0; set_cycle<SET_CYCLES; set_cycle++){
		for(int32_t target_time = 2000; target_time<=10000; target_time+=1000){				// SET di esperimenti
			logMsg("-- Experiment %d --\n", experiment_number,0,0,0,0,0);		//DEBUG
			long estimated_computations = calibrate(target_time);

			/* Compute */
			createTimestamp(T_START); 			// Start Cronometer
			compute(estimated_computations);	// Run Experiment
			createTimestamp(T_END); 			// Stop Cronometer
			saveExperiment(fd,experiment_number,estimated_computations, target_time);
			experiment_number++;
		}
	}
...
/* Try to guess the computations needed to  reach a given time target 
	based on the error made with a default computations number         */
long calibrate(int32_t target_time){
	createTimestamp(T_START);
	compute(MAGIC_NUMBER);
	createTimestamp(T_END);
	uint32_t computation_time = computeDelta();

	uint32_t delta_time = target_time-computation_time;
	double d_computations = (double)MAGIC_NUMBER;
	double d_delta_time = (double)delta_time;
	double d_computation_time = (double)computation_time;
	double d_computation_delta = ((d_computations*d_delta_time)/d_computation_time);

	long expected_computations = (long)MAGIC_NUMBER + (long)d_computation_delta;
	return expected_computations;
}
